...
Variable make_unit( const UnitType& type, int supplyAvailable, UnitData& data )
{
  ...
  case UnitType::Zealot:
    ...
    return Variable( "Zealot", "p_z", 0, supplyAvailable + 1 );
  case UnitType::Dragoon:
    data = UnitData( 125, 50, 2., (20.0/30)*24 );
    return Variable( "Dragoon", "p_d", 0, supplyAvailable + 1 );
  case UnitType::DarkTemplar:
    data = UnitData( 125, 100, 2., (40.0/30)*24 );
    return Variable( "DarkTemplar", "p_dt", 0, supplyAvailable + 1 );
  case UnitType::Reaver:
    data = UnitData( 200, 100, 4., (100.0/60)*24 );
    return Variable( "Reaver", "p_r", 0, supplyAvailable + 1 );
  case UnitType::Scout:
    data = UnitData( 275, 125, 3., (8.0/30)*24 );
    return Variable( "Scout", "p_s", 0, supplyAvailable + 1 );

    // Terran
  case UnitType::Marine:
    data = UnitData( 50, 0, 1., (6.0/15)*24 );
    return Variable( "Marine", "t_m", 0, supplyAvailable + 1 );
  case UnitType::Firebat:
    data = UnitData( 50, 25, 1., (16.0/22)*24 );
    return Variable( "Firebat", "t_f", 0, supplyAvailable + 1 );
  case UnitType::Ghost:
    data = UnitData( 25, 75, 1., (10.0/22)*24 );
    return Variable( "Ghost", "t_gh", 0, supplyAvailable + 1 );
  ...
  case UnitType::SiegeTankSiegeMode:
    ...
    return Variable( "SiegeTankSiegeMode", "t_sm", 0, supplyAvailable + 1 );
  case UnitType::Goliath:
    data = UnitData( 100, 50, 2., (12.0/22)*24 );
    return Variable( "Goliath", "t_go", 0, supplyAvailable + 1 );
  case UnitType::Wraith:
    data = UnitData( 150, 100, 2., (8.0/30)*24 );
    return Variable( "Wraith", "t_w", 0, supplyAvailable + 1 );
  case UnitType::BattleCruiser:
    data = UnitData( 400, 300, 6., (25.0/30)*24 );
    return Variable( "BattleCruiser", "t_b", 0, supplyAvailable + 1 );

    // Zerg
  case UnitType::Zergling:
    data = UnitData( 25, 0, 0.5, 15.0 );
    return Variable( "Zergling", "z_z", 0, supplyAvailable + 1 );
  case UnitType::Hydralisk:
    data = UnitData( 75, 25, 1., (10.0/15)*24 );
    return Variable( "Hydralisk", "z_h", 0, supplyAvailable + 1 );
  case UnitType::Lurker:
    data = UnitData( 75, 25, 2., (20.0/37)*24 );
    return Variable( "Lurker", "z_l", 0, supplyAvailable + 1 );
  case UnitType::Ultralisk:
    data = UnitData( 200, 200, 4., (20.0/15)*24 );
    return Variable( "Ultralisk", "z_u", 0, supplyAvailable + 1 );
  case UnitType::Mutalisk:
    data = UnitData( 100, 100, 2., (9.0/30)*24 );
    return Variable( "Mutalisk", "z_m", 0, supplyAvailable + 1 );
  case UnitType::Guardian:
    data = UnitData( 100, 100, 2., (20.0/30)*24 );
    return Variable( "Guardian", "z_g", 0, supplyAvailable + 1 );
