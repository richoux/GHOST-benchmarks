...
int main( int argc, char **argv )
{
	...
	extract_data_from_file( filename, number_variables, distances );
	
  BuilderTSP builder( number_variables, distances );
  ghost::Solver solver( builder );

  double cost;
  std::vector<int> solution;

  std::shared_ptr<ghost::Print> printer = std::make_shared<TSPPrint>();
