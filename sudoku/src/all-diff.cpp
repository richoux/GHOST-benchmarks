...
double AllDiff::expert_delta_error( const vector<Variable>& variables, const vector<unsigned int>& variable_indexes, const vector<int>& candidate_values ) const
{
	double diff = 0.0;
	auto copy_count( _count );

	// std::cout << "In AllDiff::expert_delta_error\n";
	...
	// for( int i = 0 ; i < static_cast<int>( variable_indexes.size() ) ; ++i )
	// {
	// 	diff += binomial_with_2_diff_minus_1( _count[ variables[ variable_indexes[ i ] ].get_value() - 1 ] );
	// 	diff += binomial_with_2_diff_plus_1( _count[ candidate_values[ i ] - 1 ] );
	// }

	// return diff;
}

void AllDiff::update_constraint( const vector<Variable>& variables, unsigned int variable_index, int new_value )
{
